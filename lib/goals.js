...
function distanceSqrt( dx, dy, dz) {
...
class GoalPlaceBlock extends Goal {
  ...
  isEnd (node) {
    ...
    if (distanceSqrt(headPos.x - this.pos.x, headPos.y - this.pos.y, headPos.z - this.pos.z) > this.options.range * this.options.range) return false
  ...
  getFaceAndRef (headPos) {
    for (const [face, to, ref] of this.facesPos) {
      ...
      if (block && block.position.equals(ref)) {
        if (block.face !== vectorToDirection (face.scaled(-1))) continue
        return { face, to, ref }
      }
    }
    return null
  ...
  isStandingIn (node) {
    ...
    const dy = node.y - this.pos.y
    const dz = node.z - this.pos.z
    return (Math.abs(dx) + Math.abs(dy < 0 ? dy + 1 : dy) + Math.abs(dz)) < 0.5
  }
}
